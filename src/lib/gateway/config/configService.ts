...
export const VERSION = "4.9.0"; // 重构版版本号（4.9.0：移除 opencode / qwenweb 内置提供商预设与适配器，API 中转仅保留 workbuddy / workbuddy-intl / openai / anthropic；历史 DB 数据继续可见但运行时不再支持；4.7.3：（原 2.4.0 → Node.js 重构；4.7.3：模型下拉项精简 —— 去除上下文（176k/24k）/能力（图像·推理·无工具）/默认标注，仅保留模型名 + 倍率/免费徽章（元数据仍完整透传，仅展示精简）；4.7.2：模型目录拉取 INTL 打通 + 弹层修复 —— WorkBuddy 模型目录端点统一改走 /v2 CLI 通道（/v2/enterprises/personal/models，CLI Bearer 直用；/console Web 路径仅认网页 cookie 会话，CLI Bearer 调 INTL 上游 500 无解，经用户 HAR 对照 + bundle 逆向 + 4 账户×2 host 全矩阵实测定位；INTL 18 模型含 GPT-5.x 系，CN 30 模型响应与 /console 逐字段一致）；前端：模型下拉头部去除「CLI 可用 x」仅保留「上游实时 · x 个」+ 修复弹层超出对话框卡片（Radix collisionBoundary=Dialog + 18rem 高度上限，长目录内部滚动）；4.7.1：路由候选模型目录 WorkBuddy 真实上游拉取 —— /api/console/providers/models 接入 Web 端 /console/enterprises/personal/models（Task 57 逆向实测，CLI 凭证 Bearer 可用），CLI 白名单有序过滤 + 元数据（倍率/上下文/能力）透传，前端下拉富展示（倍率/免费/默认/上下文徽章），INTL 同构端点上游 500 自然降级 derived；4.7.0：合入上游 fork（countossbot）12 commit 修复批次 —— Docker 部署套件（node22-slim standalone + GHCR workflow）+ init.sql 补齐 monthlyCostLimit/ModelPricing（全新部署缺表缺列修复）+ route config re-export 改本地 force-dynamic（standalone 构建生效）+ PRAGMA 初始化改 instrumentation 显式 await（竞态修复）+ 移除路由试跑（v4.3.1）与 provider 测试连接功能（净 -1693 行）+ 签到多选即选即存；审查修复：Dockerfile OCI source label 移至 runner stage（builder LABEL 不继承最终镜像，GHCR 关联此前失效）；4.6.4：回退 4.6.3 INTL 独立标识实验（用户决策）—— CN+INTL 两 region 统一 X-Client-Platform: "web"，与 WorkBuddy Web 端 axios 拦截器实测枚举完全一致，四通道全覆盖；4.6.3（已回退）：INTL 曾短暂改为 "workbuddy ai" 自定义值；4.6.2：WorkBuddy 上游请求统一携带使用端标识 X-Client-Platform: web —— 对齐 Web 端 axios 拦截器行为（HAR + bundle 实测），计费 client 字段按调用来源记账（Web 端带标识记 "WorkBuddy"，CLI 通道无标识记 ""）；4.6.1：/v1/responses 严格 Responses SSE 生命周期 —— 终点事件保证（completed / incomplete / failed 三类终点，永不裸 EOF）、每事件单调 sequence_number + response.in_progress、上游断流经 passthrough uag-upstream-error 标记转译为 response.failed（截断不伪装完成）、流内 error 帧检测、每流完整性日志（terminal/last_event/bytes/upstream_error/client_aborted）；4.6.0：OpenAI Responses API 入站端点 /v1/responses（Codex CLI）—— 服务端工具剥离降级（X-Gateway-Dropped-Tools 头 + warn 日志）+ tool_choice 防御降级 + tool_call 序列统一修复通道（相邻合并/孤儿丢弃/悬空补齐）；4.5.0：月度账单与预算 —— 虚拟密钥月度成本预算（monthlyCostLimit，估算口径月累计 ≥ 预算入口 429 + X-Budget-* 头组，下月 1 日重置）+ 总览月度账单卡（按密钥分组月成本报表 + byModel 明细 + 环比上月 + CSV 导出）；4.4.0：用量成本估算（ModelPricing 单价表 + 六视图成本）；4.3.2：运维监控体验升级；4.3.1：路由试跑控制台调试工具
...
export const DEFAULT_ROUTES: Record<string, RouteCandidateConfig[]> = {
  ...
  "deepseek-v4-flash": [{ provider: "workbuddy", model: "deepseek-v4-flash" }],
  "deepseek-v4-pro": [
    { provider: "workbuddy", model: "deepseek-v4-pro" },
    { provider: "workbuddy", model: "deepseek-v4.1-flash" },
  ...
  "claude-3-5-haiku-20241022": [{ provider: "workbuddy", model: "deepseek-v4-flash" }],
  "claude-3-haiku-20240307": [{ provider: "workbuddy", model: "deepseek-v4-flash" }],
  "claude-3-opus-20240229": [
    { provider: "workbuddy", model: "deepseek-v4-pro" },
    { provider: "openrouter", model: "anthropic/claude-3-opus" },
  ],
  "glm-5.2": [{ provider: "workbuddy", model: "glm-5.2" }],
  "kimi-k3-1": [{ provider: "workbuddy", model: "kimi-k3-1" }],
  "glm-5.2-intl": [{ provider: "workbuddy-intl", model: "glm-5.2" }],
};
